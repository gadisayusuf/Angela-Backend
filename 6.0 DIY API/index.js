...
app.get("/filter", async (req, res) => {
  try {
    ...
    if (filteredJokes.length === 0) {
      return res.status(404).json({ error: "No jokes found of that type." });
    }
...
app.post("/jokes", (req, res) => {
  ...
  if (!jokeText || !jokeType) {
    return res.status(400).json({ error: "Joke text and type are required." });
  }
  const newJoke = {
    id: jokes.length + 1,
    jokeText: text,
...
app.patch("/jokes/:id", (req, res)=>{
  ...
  const index = jokes.findIndex((joke) => joke.id === id);
  const { text, type } = req.body;
  if (text) jokes[index].jokeText = text;
  if (type) jokes[index].jokeType = type;
  res.json(jokes[index]);
  console.log( jokes[index]);
});
...
app.delete("/jokes/:id", (req, res)=>{
  ...
  console.log(deleted);
  res.json(deleted, {": Joke deleted successfully." });
})
//8. DELETE All jokes
